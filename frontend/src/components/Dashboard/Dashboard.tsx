...
function PlannerContent({
  ...
  const handleGenerateMealPlan = async (startDate: string, endDate: string, preferences: any) => {
    ...
    try {
      ...
      result.mealPlan.items.forEach((item: any) => {
        ...
        const meal: Meal = {
          ...
          recipe: {
            ingredients: item.recipe.ingredients.map((ing: any) => 
              `${ing.amount} ${ing.unit.value} ${ing.name}`
            ),
            instructions: item.recipe.instructions.map((inst: any) => inst.instruction),
      ...
      alert(`Successfully generated ${result.summary.totalMealsGenerated} meals for ${result.summary.daysPlanned} days!`);
